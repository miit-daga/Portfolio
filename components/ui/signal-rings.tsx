...
export const SignalRings = () => {
    ...
    // A one-off line that replaces the rotation for one speech: the city
    // greeting, or a complaint about being poked
    ...
    // Hidden while its twin runs the "Send a message" errand (mail-courier.tsx)
    ...
    // First, the one-offs: a complaint if the ship was just poked, else a
    // greeting to the visitor's city (found by the signal globe) once a session.
    useEffect(() => {
        ...
        if (!greetedRef.current && city && !/sector|space/i.test(city)) {
            greetedRef.current = true;
            setSpecialLine(
                city === "Kolkata" ? "A local! Greetings, neighbour from Kolkata." : `Greetings, traveller from ${city}.`,
            );
            return;
        }
        setSpecialLine(null);
    ...
    useEffect(() => {
        if (phase !== "descend") setPokeLanding(false);
    }, [phase]);
    ...
    // see who did it; landed, it just complains with its lights.
    const onPoke = () => {
        if (reduce || away) return;
        setPoke((n) => n + 1);
        setAlert(true);
        window.setTimeout(() => setAlert(false), 1400);
        if (phase === "orbit") {
            pokedRef.current = true;
            setPokeLanding(true);
            setPhaseIndex(SEQUENCE.findIndex((s) => s.p === "descend"));
        }
    };
    ...
    } else if (phase === "descend" && pokeLanding) {
        // Poked mid-orbit: straight down to the pad from wherever it was
        ufoAnimate = { x: 0, y: LAND_Y, scale: LAND_SCALE, rotate: 0, opacity: 1, zIndex: 20 };
        ufoTransition = { duration: 1.2, ease: [0.4, 0, 0.3, 1] };
    ...
    return (
        <div ref={rootRef} style={{ transform: `scale(${sceneScale})` }} className="pointer-events-none relative flex h-full w-full items-center justify-center overflow-visible">
            ...
            {/* UFO: click it to poke it */}
            <motion.div
                data-signal-ufo
                className={reduce ? "absolute" : "pointer-events-auto absolute cursor-pointer"}
                ...
                animate={{ ...ufoAnimate, opacity: away ? 0 : 1 }}
            ...
            >
                <motion.div
                    key={poke}
                    initial={{ rotate: 0 }}
                    animate={poke ? { rotate: [0, -16, 12, -8, 5, 0], y: [0, -3, 0, -2, 0, 0] } : { rotate: 0 }}
                    transition={{ duration: 0.75, ease: "easeOut" }}
                >
                    <Ufo reduce={reduce} hatchOpen={hatchOpen} thrust={thrust} alert={alert} />
                </motion.div>
            </motion.div>
                </>
            )}
